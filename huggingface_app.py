...
st.set_page_config(page_title="SmartSave AI", page_icon="$", layout="wide")

st.markdown("""
<style>
    .stApp { background: linear-gradient(135deg, #0A2010 0%, #0F3A1A 25%, #1A4A0D 50%, #0F3A1A 75%, #061208 100%); background-size: 400% 400%; animation: emeraldShift 10s ease infinite; }
    @keyframes emeraldShift { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }
    section[data-testid="stSidebar"] { background: linear-gradient(180deg, #061208 0%, #0A2010 100%) !important; border-right: 1px solid #22C55E !important; }
    label { color: #4ADE80 !important; font-weight: 600 !important; }
    .stTextInput > div > div > input { background-color: rgba(10,32,16,0.9) !important; color: #86EFAC !important; border: 1px solid #22C55E !important; border-radius: 10px !important; }
    .stTextInput > div > div > input::placeholder { color: #4ADE80 !important; opacity: 0.6 !important; }
    div[data-testid="stNumberInput"] input { background-color: rgba(10,32,16,0.9) !important; color: #86EFAC !important; border: 1px solid #22C55E !important; }
    div[data-testid="stNumberInput"] button { background-color: rgba(10,32,16,0.9) !important; color: #4ADE80 !important; border: 1px solid #22C55E !important; }
    .stSelectbox > div > div { background-color: rgba(10,32,16,0.9) !important; color: #86EFAC !important; border: 1px solid #22C55E !important; }
    .stButton > button { background-color: #16A34A !important; color: white !important; border-radius: 10px !important; border: none !important; font-weight: bold !important; width: 100% !important; height: 45px !important; }
    .stButton > button:hover { background-color: #15803D !important; }
    .stTabs [data-baseweb="tab-list"] { background-color: rgba(10,32,16,0.8) !important; border-radius: 10px !important; }
    .stTabs [data-baseweb="tab"] { color: #86EFAC !important; }
    .stTabs [aria-selected="true"] { background-color: #16A34A !important; border-radius: 8px !important; }
...
def load_everything():
    ...
    for key, plan in savings_plans.items():
        content = "PLAN: " + plan["full_name"] + " (" + key + ")\n" + "CATEGORY: " + plan["category"] + "\n" + "INTEREST: " + plan["interest_rate"] + "\n" + "LOCK-IN: " + plan["lock_in"] + "\n" + "TAX SECTION: " + plan["tax_section"] + "\n" + "TAX BENEFIT: " + plan["tax_benefit"] + "\n" + "WHO: " + plan["who_can_invest"] + "\n" + "RISK: " + plan["risk"] + "\n" + "BEST FOR: " + plan["best_for"] + "\n" + "TARGET: " + plan["target_audience"] + "\n" + "EXAMPLE: " + plan["example"] + "\n" + "PRO TIP: " + plan["pro_tip"]
        docs.append(Document(page_content=content, metadata={"plan_key":key,"plan_name":plan["full_name"],"category":plan["category"],"target":plan["target_audience"]}))
    for section, detail in tax_sections.items():
        docs.append(Document(page_content="TAX SECTION " + section + ": " + detail, metadata={"plan_key":"TAX_"+section,"plan_name":"Section "+section,"category":"Tax Section","target":"Everyone"}))
    for audience, plans in audience_guide.items():
        docs.append(Document(page_content="AUDIENCE GUIDE for " + audience + ": Best plans are " + ", ".join(plans), metadata={"plan_key":"GUIDE_"+audience,"plan_name":"Guide for "+audience,"category":"Audience Guide","target":audience}))
    embeddings = HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2")
    vectordb = FAISS.from_documents(docs, embeddings)
    return vectordb, savings_plans, audience_guide, comparison_data


with st.spinner("Loading SmartSave AI..."):
...
def calculate_tax_savings(annual_income, tax_slab, investments):
    rate = {"5%":0.05,"10%":0.10,"20%":0.20,"30%":0.30}.get(tax_slab, 0.20) * 1.04
    results = {}
    total_deduction = 0
    total_tax_saved = 0
    sec_80c_used = 0
...
def calculate_investment_returns(monthly_amount, years, return_percent, plan_name):
    ...
    n = years * 12
    fv = monthly_amount * (((1+r)**n - 1)/r) * (1+r) if r > 0 else monthly_amount * n
    ...
    return {"plan":plan_name,"monthly":monthly_amount,"years":years,"total_invested":round(total_invested),"maturity":round(fv),"returns":round(fv-total_invested),"multiple":round(fv/total_invested,1),"milestones":milestones}
...
#     system_prompt = "You are SmartSave AI — a friendly expert financial advisor for ALL Indians. Help " + user_type + " with savings plans, tax benefits and investment strategies. Use ONLY the knowledge base context. Give specific rupee amounts. Mention exact tax savings. Keep language simple and friendly. End with: Would you like to know more about any specific plan?"
#     response = client.chat.completions.create(model="llama-3.3-70b-versatile", messages=[{"role":"system","content":system_prompt},{"role":"user","content":"Context:\n" + context + "\n\nQuestion: " + question}])
...
def smartsave_answer(question, user_type="General"):
    ...
    context = "\n\n".join([d.page_content for d in docs])
    system_prompt = (
        "You are SmartSave AI — a friendly expert "
        "financial advisor for ALL Indians. "
        "Help " + user_type + " with savings plans. "
        "Use ONLY the knowledge base context. "
        "Give specific rupee amounts. "
...
def green_card(value, label):
    return "<div style='background:rgba(10,32,16,0.8);border:1px solid #22C55E;border-radius:10px;padding:12px 16px;text-align:center;margin:4px 0;'><div style='color:#4ADE80;font-size:20px;font-weight:bold;'>" + str(value) + "</div><div style='color:#86EFAC;font-size:11px;'>" + label + "</div></div>"


with st.sidebar:
    st.markdown("<div style='text-align:center;padding:16px;'><h2 style='color:#4ADE80;margin:0;'>SmartSave AI</h2><p style='color:#86EFAC;font-size:13px;'>Your Investment Advisor</p></div>", unsafe_allow_html=True)
    st.markdown("---")
    st.markdown("<p style='color:#4ADE80;font-weight:bold;font-size:14px;'>I am a...</p>", unsafe_allow_html=True)
    user_type = st.selectbox("Profile", ["Select profile...","IT Employee","Government Employee","Self Employed","Student","Senior Citizen","Public Sector","Freelancer"], label_visibility="collapsed")
    st.markdown("---")
        ...
        if matched:
            ...
            for plan_key in audience_guide[matched]:
                if plan_key in savings_plans:
                    ...
                    st.markdown("<div style='background:rgba(10,32,16,0.8);padding:8px;border-radius:6px;margin:4px 0;border-left:3px solid #22C55E;'><span style='color:#4ADE80;font-size:12px;font-weight:bold;'>" + plan_key + "</span><br><span style='color:#86EFAC;font-size:11px;'>" + plan["full_name"] + "</span><br><span style='color:#22C55E;font-size:11px;'>" + plan["tax_section"] + "</span></div>", unsafe_allow_html=True)
    st.markdown("---")
    st.markdown("<p style='color:#166534;font-size:11px;text-align:center;'>Built by Charan Sai N<br>AI Career Accelerator 2026</p>", unsafe_allow_html=True)


st.markdown("<div style='background:linear-gradient(135deg,#052E16,#14532D);padding:24px;border-radius:12px;text-align:center;margin-bottom:20px;border:1px solid #22C55E;'><h1 style='color:white;margin:0;font-size:32px;'>SmartSave AI</h1><p style='color:#4ADE80;margin:4px 0;'>Personalized Investment Advisor for ALL Indians</p><p style='color:#86EFAC;font-size:12px;margin:0;'>Government · Private · Self-Employed · Students · Senior Citizens</p></div>", unsafe_allow_html=True)


tab1, tab2, tab3, tab4 = st.tabs(["Chat Advisor", "Tax Calculator", "Investment Calculator", "Compare Plans"])
...
with tab1:
    ...
    # Process if last message is thinking placeholder
    if (st.session_state.messages and
        st.session_state.messages[-1]["content"] == "SmartSave AI is thinking..." and
        len(st.session_state.messages) >= 2):
        
        question = st.session_state.messages[-2]["content"]
        with st.spinner(""):
            reply = smartsave_answer(question, user_type)
        st.session_state.messages[-1]["content"] = reply
        st.rerun()

    for msg in st.session_state.messages:
        if msg["role"] == "user":
            st.markdown("<div style='display:flex;justify-content:flex-end;margin:8px 0;'><div style='background:#14532D;color:#DCFCE7;padding:10px 16px;border-radius:18px 18px 4px 18px;max-width:80%;font-size:14px;border:1px solid #22C55E;'>" + msg["content"] + "</div></div>", unsafe_allow_html=True)
        else:
            st.markdown("<div style='display:flex;justify-content:flex-start;margin:8px 0;align-items:flex-start;'><div style='background:#16A34A;color:white;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;margin-right:10px;font-size:16px;flex-shrink:0;'>$</div><div style='background:rgba(10,32,16,0.9);color:#86EFAC;padding:12px 16px;border-radius:4px 18px 18px 18px;max-width:80%;font-size:14px;border:1px solid #22C55E;'>" + msg["content"] + "</div></div>", unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    col1, col2 = st.columns([4, 1])
    with col1:
        user_input = st.text_input("Ask", placeholder="Ask about savings plans, tax benefits, investment advice...", label_visibility="collapsed", key="chat_input_" + str(st.session_state.input_key))
    # UPDATE text input ✅
    # with col1:
    #     user_input = st.text_input(
    #         "Ask",
    #         placeholder="Ask about savings plans...",
    ...
    # current_input = st.session_state.get("chat_input_" + str(st.session_state.input_key), "")
    # msg_to_process = ""
    ...
    msg_to_process = ""
    if send and current_input:
        msg_to_process = current_input
    elif st.session_state.pending_msg:
        msg_to_process = st.session_state.pending_msg
        st.session_state.pending_msg = ""
    elif current_input and current_input != st.session_state.last_processed:
        msg_to_process = current_input
    ...
    #         reply = smartsave_answer(msg_to_process, user_type)
    ...
    if msg_to_process:
        ...
        st.session_state.messages.append({
            "role": "assistant",
            "content": "SmartSave AI is thinking..."
        })
        st.rerun()
...
with tab2:
    ...
    if st.button("Calculate Tax Savings"):
        ...
        with col3:
            st.markdown(green_card(str(round((result["total_tax_saved"]/income)*100,1)) + "%", "of Income Saved"), unsafe_allow_html=True)
        ...
        for plan, data in result["breakdown"].items():
            st.markdown("<div style='background:rgba(10,32,16,0.8);padding:8px 12px;border-radius:6px;margin:4px 0;border-left:3px solid #22C55E;display:flex;justify-content:space-between;'><span style='color:#86EFAC;'>" + plan + "</span><span style='color:#4ADE80;font-weight:bold;'>Rs. " + "{:,}".format(data["tax_saved"]) + " saved</span></div>", unsafe_allow_html=True)
...
with tab4:
    ...
    if st.button("Compare Now"):
        ...
        if result:
            ...
            col1, col2 = st.columns(2)
            with col1:
                st.markdown("<div style='background:rgba(10,32,16,0.8);padding:16px;border-radius:10px;border:2px solid #22C55E;'><h4 style='color:#4ADE80;text-align:center;'>" + plan1 + "</h4>", unsafe_allow_html=True)
                for field, label in zip(fields, labels):
                    st.markdown("<div style='padding:6px 0;border-bottom:1px solid rgba(34,197,94,0.2);'><span style='color:#86EFAC;font-size:12px;'>" + label + "</span><br><span style='color:#DCFCE7;font-size:13px;font-weight:bold;'>" + str(p1.get(field,"N/A")) + "</span></div>", unsafe_allow_html=True)
                st.markdown("</div>", unsafe_allow_html=True)
            with col2:
                st.markdown("<div style='background:rgba(10,32,16,0.8);padding:16px;border-radius:10px;border:2px solid #4ADE80;'><h4 style='color:#4ADE80;text-align:center;'>" + plan2 + "</h4>", unsafe_allow_html=True)
                for field, label in zip(fields, labels):
                    st.markdown("<div style='padding:6px 0;border-bottom:1px solid rgba(34,197,94,0.2);'><span style='color:#86EFAC;font-size:12px;'>" + label + "</span><br><span style='color:#DCFCE7;font-size:13px;font-weight:bold;'>" + str(p2.get(field,"N/A")) + "</span></div>", unsafe_allow_html=True)
                st.markdown("</div>", unsafe_allow_html=True)


st.markdown("<div style='text-align:center;margin-top:30px;color:#166534;font-size:11px;'>SmartSave AI · Built by Charan Sai N · AI Career Accelerator 2026 · Not financial advice — consult a SEBI registered advisor for personal decisions</div>", unsafe_allow_html=True)
